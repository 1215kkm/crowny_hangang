...
## 18. 프로젝트별 테마 컬러 (강디2 기본 제안 룰)

이 문서의 **`--primary` / `--secondary`** (보라→분홍) 는 **Crowny Class 플랫폼 자체의 기본 컬러**다.
강팀이 *다른 프로젝트*를 만들 때는 그 *주제에 맞는 테마 컬러*를 강디2가 **먼저 제안**한다.

### 제안 규칙

1. **새 프로젝트 시작 회의에서 강디2가 가장 먼저 한 턴** — 프로젝트 주제·타겟 사용자·경쟁군을 보고 *Primary + Secondary 한 쌍*을 제안. 그라데이션 각도(135deg)·구조·토큰 이름은 그대로 유지, **HEX 값만** 갈아끼움.
